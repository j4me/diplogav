...
sum:=Sum+1;
     StaticText1.Caption:='  Что включает в себя 1ой тип объектов мониторинга:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=false;
...
Radiobutton2.Caption:='среды распространения сигналов;';
...
Radiobutton2.Visible:=false;
Radiobutton3.Visible:=false;
StaticText1.Caption:='  Что наиболее важным является для третьего типа объектов мониторинга:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
CheckBox1.Visible:=true;
CheckBox1.Checked:=false;
...
CheckBox7.Visible:=false;
CheckBox7.Checked:=false;
CheckBox1.Caption:='система телефонной связи, включая подсистему сигнализации ОКС7 и абонентские каналы ISDN;';
//ChechBox1.WrapText:=true;
CheckBox2.Caption:='аналоговые каналы первичной сети;';
CheckBox3.Caption:='потоки иерархий PDH и SDH.';
...
Radiobutton2.Visible:=false;
Radiobutton3.Visible:=false;
StaticText1.Caption:='  Кто из пользователей не имеет ограничений в правах доступа к данным:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
CheckBox1.Visible:=true;
CheckBox1.Checked:=false;
...
CheckBox3.Caption:='Администратор.';
end;
19:
begin
...
Radiobutton2.Visible:=true;
Radiobutton3.Visible:=true;
StaticText1.Caption:='  Сколько дочерних окон имеет приложение «Просмотр»:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
CheckBox1.Visible:=false;
CheckBox1.Checked:=false;
...
CheckBox7.Visible:=false;
CheckBox7.Checked:=false;
CheckBox1.Caption:='об авариях групп контроллеров;';
CheckBox2.Caption:='о системных сообщениях;';
CheckBox3.Caption:='об объектовых устройствах.';
...
Radiobutton2.Visible:=false;
Radiobutton3.Visible:=false;
StaticText1.Caption:='  Для чего применяется 2х битный контроль:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
CheckBox1.Visible:=true;
CheckBox1.Checked:=false;
...
CheckBox7.Visible:=false;
CheckBox7.Checked:=false;
CheckBox1.Caption:='для восстановления работы сигнала;';
CheckBox2.Caption:='для проверки текущего состояния датчика;';
CheckBox3.Caption:='для сигналов объектовых устройств.';
end;
22:
begin
   if ((CheckBox3.Checked) ) then
...
StaticText1.Caption:='  Гарантируемая  максимальная  протяженность  физической  пары проводов для УСИ 56Т СЛ:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=false;
...
if Radiobutton2.Checked then
     sum:=Sum+1;
   StaticText1.Caption:='  Сколько внешних модулей расширения можно подключить к одному КУБу:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=false;
...
begin
 if Radiobutton1.Checked then
     sum:=Sum+1;
StaticText1.Caption:='  Распределительный  статив ( РС)  предназначен  для  понижения  и стабилизации давления воздуха,'+' вырабатываемого блоком осушки КСУ-Э, распределения его по кабельным оболочкам, измерения расхода воздуха в  каждой  из'+'  них  и  автоматического  контроля  за  превышением допустимого предела этого расхода:';
...
if Radiobutton2.Checked then
     sum:=Sum+1;
StaticText1.Caption:='  Для чего предназначен внешний модуль расширения МСИ485:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=false;
...
Radiobutton2.Visible:=true;
Radiobutton3.Visible:=true;
StaticText1.Caption:='  Максимальная удалённость выносных блоков БИК от основного блока БИК составляет:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
CheckBox1.Visible:=false;
CheckBox1.Checked:=false;
...
CheckBox7.Visible:=false;
CheckBox7.Checked:=false;
Radiobutton1.Caption:='70  метров;';
Radiobutton2.Caption:='100  метров;';
Radiobutton3.Caption:='50  метров.';
...
StaticText1.Caption:='  Какие данные указываются в акте при повреждении линии связи или лини радиофикации (для юридических лиц):';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=true;
...
Radiobutton2.Visible:=False;
Radiobutton3.Visible:=False;
CheckBox1.Caption:='название предприятия;';
CheckBox2.Caption:='должность и ФИО (виновного);';
CheckBox3.Caption:='характер, место и время происшествия;';
CheckBox4.Caption:='все выше перечисленные.';
end;
28:
begin
if CheckBox4.Checked then
     sum:=Sum+1;
StaticText1.Caption:='  Сочетанием, каких клавиш можно открыть окно «Состояние направлений»:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=false;
...
if Radiobutton1.Checked then
     sum:=Sum+1;
StaticText1.Caption:='  Какие функции выполняет диспетчер, при решении задачи № 3:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=true;
...
CheckBox2.Caption:='формирует суточный  рапорт о состоянии сетей связи  и передает его руководству ГЦТ, ОДС РЦУСС;';
CheckBox3.Caption:='контролирует выдачу ключей аварийной и охранной  бригаде при выезде на объект;';
CheckBox4.Caption:='оповещает заинтересованных потребителей (подразделений) в случае проведения работ на сетях связи;';
CheckBox5.Caption:='собирает и  анализирует информацию  о повреждениях на сетях связи;';
CheckBox6.Caption:='принимает экстренные меры  по предупреждению и устранению   аварийных ситуаций на ГЦТ.';
end;
30:
begin
if ((CheckBox1.Checked) and (CheckBox2.Checked) and (CheckBox4.Checked) and (CheckBox5.Checked)) then
     sum:=Sum+1;
StaticText1.Caption:='  В каком окне осуществляется просмотр сводной информации о пребывании какого-либо сигнала в различных состояниях:';
Statictext1.Font.Size :=35;
StaticText2.Caption := inttostr(s)+'/30';
StaticText3.Caption := inttostr(sum);
CheckBox1.Visible:=false;
...
Radiobutton2.Visible:=true;
Radiobutton3.Visible:=true;
Radiobutton1.Caption:='«Статистика сигнала»;';
Radiobutton2.Caption:='«Журнал горячей линии»;';
Radiobutton3.Caption:='«Сигналы».';
